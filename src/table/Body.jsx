...
import {Cell, CellButton} from 'components/Table/Cells'
...
function row( json_array, row_idx ) {
    ...
    const row = []
    for (var i=0; i < json_array.columns.length; i++ ){
        row.push(
          <Cell
            {...props}
            col={i}
    ...
    var rowOnClick = null
    if( json_array.rowOnClick !== undefined ){
      rowOnClick = () => json_array.rowOnClick({
        row: props.row,
        row_data: data
      })
    }
